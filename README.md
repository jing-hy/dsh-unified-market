...
# 或 GitHub 源：
dsh plugin --profile web-desktop add github:<your-gh-username>/dsh-unified-market
```
...
作为内置插件，市场自身跟随上游版本更新的方式：

1. 在 `main.js` 的 `PLUGIN_UPDATE_SOURCES` 启用对应条目（二选一）：
   ```js
   'unified-market': { npm: 'dsh-unified-market' },          // 已发布 npm
   // 'unified-market': { github: '<your-gh-username>/dsh-unified-market' },
   ```
2. 之后由官方「内置插件更新」（启动 20s 后 + 每 6 小时检测、可开自动更新）负责
   更新到覆盖层并重启服务生效。
...
- **npm**：`npm publish dsh-unified-market-0.2.0.tgz`（包名在 npm 已确认可用；
  发布前把 `package.json` 的 `repository`/`homepage` 的 `<your-gh-username>` 换成真实 GitHub 用户名）。
- **GitHub**：创建同名仓库后 `git push origin main`。
